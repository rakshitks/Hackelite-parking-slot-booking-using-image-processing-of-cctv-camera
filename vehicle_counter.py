...
class VehicleCounter(object):
    def __init__(self, shape, divider,fb):
    ...
    def update_count(self, matches, output_image = None):
        if self.arr_slots is None:
            self.arr_slots = self.fire.get_slots()
        ...
        for id_ in removed:
            ...
            l = len(id_.positions)
            if l >5:
                num_free_slots = self.fire.get_free(self.arr_slots)
                free_slots = self.fire.get_free_slots(self.arr_slots)
                if num_free_slots == 0:
                    print('Parking slots are full! Sorry :(')
                    return -1
                else:
                    slot= free_slots[0]
                    self.arr_slots[slot] = 1
                    self.fire.set_slot(slot,1)
